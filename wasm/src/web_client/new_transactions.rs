...
impl WebClient {
    ...
    pub async fn new_swap_transaction(
        ...
        if let Some(client) = self.get_mut_inner() {
            ...
            let swap_transaction_execution_result = client.new_transaction(swap_transaction_request).await.unwrap();
            let result = NewTransactionResult::new(
                swap_transaction_execution_result.executed_transaction().id().to_string(),
                swap_transaction_execution_result.created_notes().iter().map(|note| note.id().to_string()).collect()
            );
