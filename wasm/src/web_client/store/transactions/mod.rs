...
use miden_objects::{
    accounts::AccountId,
    assembly::ProgramAst,
    transaction::{OutputNotes, TransactionId, TransactionScript},
    Digest, Felt,
};
...
use crate::web_client::store::notes::utils::{
    insert_input_note_tx, insert_output_note_tx, update_note_consumer_tx_id,
};
...
impl WebStore {
    ...
    pub(crate) async fn mark_transactions_as_committed(
        ...
        let transactions_to_commit_as_str: Vec<String> =
            transactions_to_commit.iter().map(|tx_id| tx_id.inner().into()).collect();

        let promise =
            idxdb_mark_transactions_as_committed(block_num_as_str, transactions_to_commit_as_str);
        let js_value = JsFuture::from(promise).await.unwrap();
