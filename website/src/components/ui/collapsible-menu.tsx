    ...
    <div className="mb-2 min-h-[48px] list-none last:mb-0">
      {dropdownItems?.length ? (
        <>
          ...
          >
            <span className="z-[1] flex items-center mr-3">
              <span className="mr-3 ml-3">{icon}</span>
              {name}
            </span>
            <span
            ...
            <ul ref={ref}>
              {dropdownItems.map((item, index) => (
                <li className="first:pt-2" key={index}>
                  <ActiveLink
                    href={item.href}
                    className="flex items-center rounded-lg p-3 text-sm text-gray-500 transition-all before:h-1 before:w-1 before:rounded-full before:bg-gray-500 hover:text-brand pl-6 before:mr-5 dark:hover:text-white"
                    activeClassName="!text-brand dark:!text-white dark:before:!bg-white before:!bg-brand before:!w-2 before:!h-2 before:-ml-0.5 before:!mr-[18px] !font-medium"
                  >
                    {item.name}
                  </ActiveLink>
                </li>
        ...
        >
          <span className="relative z-[1] mr-2">{icon}</span>
