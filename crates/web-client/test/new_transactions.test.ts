...
export const customTransaction = async (
  ...
  return await testingPage.evaluate(
    async (_assertedValue: string, _withCustomProver: boolean) => {
      ...
      let compiledNoteScript = await client.compile_note_script(noteScript);
      ...
      let transactionScript = await client.compile_tx_script(txScript);
      let noteId = note.id();
      let noteArgsCommitment = window.Rpo256.hash_elements(feltArray); // gets consumed by NoteIdAndArgs
      let noteIdAndArgs = new window.NoteIdAndArgs(
        noteId,
        noteArgsCommitment.to_word()
      );
      let noteIdAndArgsArray = new window.NoteIdAndArgsArray([noteIdAndArgs]);
      let adviceMap = new window.AdviceMap();
      let noteArgsCommitment2 = window.Rpo256.hash_elements(feltArray);
      adviceMap.insert(noteArgsCommitment2, feltArray);

      let transactionRequest2 = new window.TransactionRequestBuilder()
        .with_authenticated_input_notes(noteIdAndArgsArray)
        .with_custom_script(transactionScript)
        .extend_advice_map(adviceMap)
        .build();

      // Execute and Submit Transaction
      let transactionResult2 = await client.new_transaction(
        walletAccount.id(),
        transactionRequest2
      );
      ...
      if (_withCustomProver) {
        ...
        await client.submit_transaction(transactionResult2);
      }

      await window.helpers.waitForTransaction(
        transactionResult2.executed_transaction().id().to_hex()
      );
    },
    assertedValue,
    withCustomProver
...
export const selectProver = async (): Promise<TransactionProver> => {
  if (window.remoteProverUrl != null) {
    return window.TransactionProver.new_remote_prover(window.remoteProverUrl);
  } else {
